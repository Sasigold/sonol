...
## 7. Offline and the PWA

Only **one** mutation is queued: completing / uncompleting a station. It is the
only thing done away from a desk. Admin writes fail loudly, on purpose.

`src/lib/offline-queue.ts` holds one record **per station**, storing the
_intent_ plus the `baseline` the server believed before the first offline tap.
Repeated taps collapse: mark → undo → mark replays as one call, mark → undo
replays as nothing. The stored baseline always wins over the one a caller
passes — after the first tap the caller is reading an optimistic row.

Rejections are classified once, in `classifyMutationError` (`src/lib/errors.ts`):
`offline` keeps the record and does **not** count against the retry bound;
`permanent` drops it and tells the user; `unknown` is retried, bounded. Poison
records die on either bound — 5 attempts **or** 24 hours.

> **`useToggleStation` sets `networkMode: 'always'`. Do not remove it.**
> TanStack Query's default is to PAUSE a mutation while offline: the request
> is never sent, `onError` never fires, and the tap is held in memory only —
> lost on reload, which is exactly when a phone in a vehicle loses it. With the
> default, nothing ever reached the durable queue. Guarded by a test in
> `useStations.test.tsx`.

**Sign-out clears both the queue and the Supabase response cache.** A queued
tap replays as whoever is signed in, and the Workbox cache keys on URL while
ignoring the `Authorization` header — on a shared tablet either one hands
worker A's data to worker B. `purgeUserData` in `AuthProvider` does both.

The service worker precaches the build. `globIgnores` drops the Heebo `math`
and `symbols` subsets (no glyph in this app comes from them) and the manifest
icons, which vite-plugin-pwa precaches itself — listing them twice is a real
duplicate, not a no-op.

Updates are `autoUpdate` **plus** `onNeedReload`, which suspends the plugin's
own reload and hands the timing to `UpdatePrompt`: reload at once if the queue
is empty, otherwise offer a toast and wait. Reloading with taps outstanding
refetches the server's answer and visibly un-marks stations the worker knows
they marked.

Icons are generated by `npm run gen:icons` (Playwright renders `favicon.svg`;
there is no `sharp` here) and committed.
...
npm run gen:icons    regenerate the PWA icons from favicon.svg
npm run verify       all of the above — run before every commit
npm run test:e2e     Playwright — WRITES TO THE DATABASE, see below
```

`test:e2e` refuses to run without `E2E_ALLOW_WRITES=1`. The admin flow calls
`reset_round`, which clears every completion and zeroes every counter in the
project `.env` points at. Point it at a disposable project.
